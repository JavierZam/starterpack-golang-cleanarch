...
docker-run-db: docker-stop-db
	...
	# --- FIX: Adding a longer sleep to allow PostgreSQL entrypoint to finish its sequence ---
	sleep 5 # Wait for 5 seconds to let the DB fully settle
	# --- END FIX ---
	@echo "Database is ready. Running initial schema setup..."
	# --- FIX: Use 'cat' to pipe SQL content into psql stdin ---
